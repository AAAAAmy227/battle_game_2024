...
void Blitz::TankMove(float move_speed, float rotate_angular_speed) {
  ...
  if (fire_count_down_ == 0) {
    ...
    if (player) {
      ...
      if (input_data.mouse_button_down[GLFW_MOUSE_BUTTON_LEFT]) {
        ...
        GenerateBullet<bullet::MPBullet>(
  ...
  if (doge_count_down_ == 0) {
    ...
    if (player) {
      ...
      if (input_data.key_down[GLFW_KEY_R]) {
        auto velocity = Rotate(glm::vec2{0.0f, 20.0f}, turret_rotation_);
        GenerateBullet<bullet::MPBulletNoHarm>(
            position_ + Rotate({0.0f, 1.2f}, turret_rotation_),
            turret_rotation_, GetDamageScale(), velocity);
        doge_count_down_ = kTickPerSecond / 10;  // Fire interval 1 second.
      }
    }
  }
  if (doge_count_down_) {
...
bool Blitz::IsHit(glm::vec2 position) const {
...
float Blitz::GetHealthScale() const {
  return 0.5f;
}

float Blitz::GetSpeedScale() const {
  return 5.0f;
}
/*
...
const char *Blitz::UnitName() const {
  return "Blitz";
}

const char *Blitz::Author() const {
  return "Ledner";
}
}  // namespace battle_game::unit
